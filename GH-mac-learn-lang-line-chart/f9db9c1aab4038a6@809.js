...
md`# GH Machine Learning Languages

Percentage of Github Repositories by language. Data: [GitHub](https://github.com/search/advanced)<br>Source: Mike Bostock - [Line Chart, Multiple Series](https://observablehq.com/@d3/multi-line-chart)`
)}

function _2(d3){return(
d3.schemeSet3.splice(3, 1)
)}
...
LineChart(data.slice(42,).filter(d => Tags.includes(d.language)), {
  x: d => d.date,
  y: d => d.percent,
  z: d => d.language,
  yLabel: "% of GitHub Repositories",
  width,
  height: 500
})
)}
...
Array.from(new Set(data.map(d => d.language))).sort()
)}
...
d3.csvParse(await FileAttachment("gh_machine_learning_languages_results.csv").text(), d3.autoType)
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["gh_machine_learning_languages_results.csv", {url: new URL("https://raw.githubusercontent.com/aggarwalc/le-data-2.0-DATA/main/GitHub/gh-machine-learning-languages/gh_machine_learning_languages_results.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  ...
  main.variable(observer()).define(["Swatches","d3","tags"], _4);
